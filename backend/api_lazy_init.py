#!/usr/bin/env python3
"""
RAG Chatbot API Implementation - Lazy Initialization Version
This script provides API endpoints for querying the RAG knowledge base.
The clients are initialized lazily to avoid startup failures.
"""
...
# Global variables to hold clients (initialized lazily)
_co_client = None
_qdrant_client = None

def get_cohere_client():
    """Get or create Cohere client lazily"""
    global _co_client
    if _co_client is None:
        cohere_api_key = os.getenv("COHERE_API_KEY")
        if not cohere_api_key:
            raise ValueError("COHERE_API_KEY not found in environment variables")
        _co_client = cohere.Client(cohere_api_key)
    return _co_client
...
def health_check():
    ...
    try:
        ...
        # Return a health status even if Qdrant is not accessible
        return HealthResponse(status="partially_healthy", collections=[])
